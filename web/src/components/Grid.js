...
export const Grid = () => {
  ...
  return (
    <table tw="w-full h-full flex-auto bg-gray-200 flex flex-col">
      <ColumnHeading />
      <tbody data-qa={`selected cell id: ${selectedCell.id}`}>
        {ROWS.map((rowId) => (
          <tr key={rowId} tw="w-full h-10 flex-auto bg-gray-200 flex">
            <HeadingCell data-qa={`row heading ${rowId}`}>{rowId}</HeadingCell>
            {COLUMNS.map((colId) => {
              ...
              return (
                <CellContainer
                  ...
                  refIndex={refIndex}
                >
                  {loadingCellId === cellId && <LoadingSnake data-qa={`loadingSnake`}/>}
                  <Cell
...
const ColumnHeading = React.memo(() => (
  <thead>
    <tr tw="w-full h-8 flex-auto bg-gray-200 flex">
      <HeadingCell />
      {COLUMNS.map((x) => (
        <HeadingCell key={x} data-qa={`column heading ${x}`}>{x}</HeadingCell>
      ))}
    </tr>
  </thead>
));
